...
publish:
	git tag -s v0.3.14 -m "fncmp mmm pi v0.3.14" && \
	git push --tags && \
	GOPROXY=proxy.golang.org go list -m github.com/kitkitchen/fncmp@v0.3.14

lookup:
	curl https://sum.golang.org/lookup/github.com/kitkitchen/fncmp@v0.3.14